...
<%--
	...
	1-2-3-4-2-3-4-2-3-4
 	...
 	varStatus : 루프변수 하나의 객체라서 그 안에 필드라는것들이 있다. 
 		index - 현재 인덱스 값 
 		count - 루프 실행 횟수 
...
<c:forEach begin="0" end="10" var="i" varStatus="loop" >
	${i} : 반복시킬문장 ${loop.index} / ${loop.count} <br>
</c:forEach>

<!-- step 속성은 default 가 1이다 -->
<h3>향상된 for 문 형태</h3>
<%
...
<%
	...
	//Map, list? 
	//map : 순차적으로 접근이 아닌, 특정 키에의해 원하는 값을 확인하고 싶을 떄 
	// list : 순차적으로 접근하고 싶을 떄 
...
%>
	...
	<a href="<c:url value="/registUser" />">사용자 등록</a> <br>
	<a href="${a}">사용자 등록</a> <br>
	var 속성을 적용하지 않은 경우 : 태그를 작성한 곳에 출력 <c:url value="/userRegist" /><br>
	var 속성을 적용한 경우 : var 속성으로 저장만 된다(el 을 통해 활용)	${url} <c:url value="/userRegist" var="url"/>
