...
public class FileUploadServlet extends HttpServlet {
	...
	//userid 안나왔는데 Multipartconfig 를 어노테이션해주면 -> userid 이름은 나온다 ->이 파일정보는 처리한것(멀티파트) file 이름은아직안나옴
	Part part = req.getPart("profile"); 
	//content-Disposition 은 해더에 해당하는 정보다. 
	if(part.getSize() > 0) {
		//사용자가 업로드한 파일 이름 
		String filename = FileUtil.getFileName(part.getHeader("Content-Disposition"));
		//서버에 저장할 떄 다른 사용자의 파일과 중복되지 않도록 생성한 파일이름
		String realFileName = UUID.randomUUID().toString(); 
		part.write("d:\\upload\\" + filename);
	}
	
	
	//복수개의 파일을 업로드 하고 파트 이름을 모를때 collection 이용하기
